...
| | |
|---|---|
| **🌐 全网爬取** | 每次构建过一遍 **20,891 个候选**——npm 上带 `dsh-plugin` 或 `deepseek-harness` 的包，加上把它们用作 topic 的 GitHub 仓库。不用提交，不用排队。 |
| **🧹 严格筛查** | **拒掉 11,514 个，占看过的 55%。** 十七种机械判定（下图列出），每条拒收都附一句作者能读懂的理由。 |
| **🔌 依赖检测在你本机** | **2,513** 条收录声明了 peer 模块。解析发生在你自己的 profile 上——跟 dsh loader 挂载时问的是同一个问题——所以只有在**你这里**真的缺，卡片才显示**不兼容**。 |
| **🗓️ 每日更新** | 03:17 UTC 构建，结果提交进 git。新插件次日早上上架，仓库消失的以同样方式下架。 |
| **🗂️ 七个分类** | **工具** 4,584 · **界面** 2,356 · **集成** 777 · **其他** 565 · **模型服务** 463 · **工作流** 401 · **主题** 231。作者自己声明 `dsh.catalog` 就用他选的。 |
