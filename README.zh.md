...
- **🧹 严格筛查** —— 每次构建都会对所有候选插件进行自动检查。未通过的插件会逐一记录名称，
  ...
  徽章分别显示当前收录和过滤的数量。
- **🔌 本地依赖检查** —— 商店会逐一检查插件必需的 peer 依赖：先在你使用的 profile 中查找，
  再排除 dsh 网页端自带的模块；作者声明了支持的 dsh 版本或 profile 时，也会和你实际运行的比对。
  只有本地确实缺少依赖，或作者声明不支持你当前的环境时，插件卡片才会显示**不兼容**。
...
flowchart TB
  ...
  subgraph SHELVE["3 · 上架 —— 记录插件信息"]
    direction LR
    CAT["归入七个分类之一"]
    PEER["仅记录必需的 peer 依赖名称，<br/>不记录版本范围"]
  end
...
flowchart LR
  ...
  HOST ==> DEP{"6 · 依赖检查<br/>在当前 profile 和网页端中<br/>逐一查找 peer 依赖"}
...
- **兼容性取决于本地环境。** 目录只记录必需 peer 依赖的**名称**，不记录版本范围。几乎所有 dsh 插件
  都将版本范围声明为 `"*"`，而 Harness 自身的预发布版本不满足普通版本范围，按版本校验会把本可
  正常运行的插件误判为不兼容。因此，商店会在你使用的 profile 以及 dsh 网页端自带的模块中，检查这些
  依赖是否都已具备。
