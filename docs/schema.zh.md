...
每条条目——无论来自 npm 还是 GitHub——还带一个 `peers` 字段：插件在 `peerDependencies` 里声明的**必需** peer 的名字，从不包含版本范围；一个必需 peer 都没有时不带这个字段。在 `peerDependenciesMeta` 里标了 `optional: true` 的 peer 不会被记录，因此永远不会引出提示。从 GitHub 仓库上架的条目，要等构建下次重新读取该仓库时才会补上这个字段——每个仓库只补读一次，分摊在之后若干天的每日构建里。读者自己的 dsh 会拿每个名字既去匹配本机的安装，也去查它网页端自己提供的模块，只有两边都提供不了的名字才会提示，商店据此在有人安装一个自己 harness 跑不起来的插件之前，先打上标记。这只是提示，从不拒绝：确认之后照样可以安装。

如果你的 manifest 声明了 `dsh.compatibility`，目录也会把它记下来：`dsh` 是一个 semver 范围，写明你支持的 dsh 版本；`profiles` 是你支持的 profile 名。两者都可选，也可以只写其中一个；其中一半格式不对，只丢掉那一半，不连累另一半。构建只负责抄录，不做判断——由读者自己的 dsh 拿它和自己实际运行的版本与 profile 比对，有一项对不上，商店就给出提示，并写明两边：你声明的是什么，读者实际运行的又是什么。预发布版本同样算数，`>=0.1.0` 也接受 `0.1.5-rc.3`；dsh 解析不了的范围不做判断，也不去猜。这只是提示，从不拒绝安装；什么都不声明也不等于声明不兼容，不会产生任何提示。
