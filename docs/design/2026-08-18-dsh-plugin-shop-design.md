...
**Amendment (2026-09-01, harness compatibility): entries gain an optional `peers` field.** `peers` is an array of the package's declared `peerDependencies` names; present only when the package declares any. It is emitted at `PEERS_SCHEMA_VERSION` and above; `CATALOG_SCHEMA_VERSION` remains 5. The reasoning and implementation details are in `docs/design/2026-09-01-harness-compatibility.md` (§2).
