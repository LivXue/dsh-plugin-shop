...
| Install succeeded but `bundles` unchanged | Three different things, which the profile manifest tells apart: (a) the package IS a dependency — it installed and declares no `dsh.bundle`, the library-that-looked-like-a-plugin the gate should have caught; (b) the name is absent and the entry carries a `subdir` — a monorepo subpackage whose `&path:` never reached pnpm, so another package landed under its own name; (c) the name is absent with no subpackage in play — nothing installed under it | Report WHICH, read from the manifest. A stale catalog is named only in case (c), the one where it is a real candidate; naming it in (a) and (b) sends the reader to refresh a catalog that was already correct |
