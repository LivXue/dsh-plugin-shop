...
1. **Page the oversized cell to the window.** `pageCell` already takes a `pastWindow` mode and the window sweep already uses `'stop'` for exactly this reason; an oversized refinement cell would take the same mode and let the coverage arithmetic account for what the cell cannot reach, as it already does for the keyword itself. Measured two ways, and the gap between them IS option 2's contribution: the partition alone — window sweep plus refinement cells, no publisher axis — leaves **10 against a cap of 10**, while the shipped harvest run end to end against the live registry, with the committed vocabulary and the 500-probe slice its cursor names, leaves **5**: 6,437 enumerated of an answered 6,442, recovering 1,187 of 1,192 past the window, in 979s. So it clears the cap with half of it in hand rather than none — and the half is the publisher cells', not this option's. What option 1 structurally cannot reach is the oversized cell's own 198-name overflow: it is bounded by that cell's window, which is the same wall one level down.
...
**Option 1 is implemented; option 2 is not.** The margin it leaves is measured rather than assumed, and it is thin: of the oversized cell's own bottom 250 names, **0.8%** carry `dsh` and no other refinement, and that cell's overflow grows with the keyword, so the residual climbs on the order of half a name a day. From the measured 5, that is a week or two before the cap is met again — longer than the zero-margin reading an earlier draft of this paragraph gave, because it was written from the partition-only figure and the publisher slice had not been run. Two weeks is the horizon the next decision has to be taken inside, which is why option 2 is written down here rather than left to whoever meets the red build next.
