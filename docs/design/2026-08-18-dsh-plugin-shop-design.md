...
Two boundary conditions apply to that calculation. **Count/paging noise is independent of the window:** an aggregate shortfall of at most `MAX_SEARCH_SHORTFALL` (3) remains tolerated and reported on either side. A total of 5,251 can be an overstatement of 5,250 real names, so treating it as a proven one-name tail would falsely diagnose a 0% recovery. Larger gaps still require the window to come within that same three-name allowance and must pass both the rate floor and the residual cap.

**The retry window is a union of two observations, not one fixed ranking.** Its distinct-name count can exceed 5,250. Recovery is therefore `enumerated − min(windowNames.size, SEARCH_WINDOW)` for an over-window keyword, counting names reached by either refinement cells or changes in the retry's ranking beyond a single window's contribution. The window's uncapped measured count is still used to detect missing reachable results. For example, with a total of 5,407 and 5,400 names collected, moving 20 already recovered names into the retry window must leave recovery at 150/157; subtracting all 5,270 window names would incorrectly lower it to 130/157. The build report names the allowance used rather than claiming the missing names' cause is known.
