...
The residual cap is bracketed, not chosen: **at least 7**, the residual the `sayedev` family produced (1 to 7 in a day), which must be absorbed rather than redden the build; and **under 15**, the size of the one real partition gap this repo has measured, which must never be absorbed. It shipped at 25 on the reasoning that the family needed twenty-plus of headroom and that the two magnitudes were therefore incompatible — the family's cost is a residual of 7, so they are compatible, and 25 sat above the threshold the repo says must not be crossed. The rate floor cannot cover the gap in its place: a 0.9 floor permits 10% of the tail, so at 157 unreachable a fifteen-name gap is a rate of 0.904 and passes.
