...
(Amended 2026-09-24: by that rule `compatibility` now closes the npm optional
block, after `installSize`, and a github entry carries `installSize`, then
`peers`, then `compatibility`, then `tier` — `peers` reached this channel after
`installSize` had. Both orders stay pinned by `tier.test.ts`.)
...
**Amendment (2026-09-01, harness compatibility): entries gain an optional `peers` field.** `peers` is an array of the names of the package's REQUIRED `peerDependencies`, carried by npm and github entries alike; present only when the package requires any. **Amended 2026-09-03: it is emitted at every schemaVersion.** `PEERS_SCHEMA_VERSION` and `SHOP_CATALOG_V6` are removed — the field is additive and an older client's non-strict zod strips it, so the gate was never a compatibility one, and bumping the emitted version to 6 would have thrown in every client capping at 5 for a field those clients ignore. `CATALOG_SCHEMA_VERSION` remains 5. The reasoning and implementation details are in `docs/design/2026-09-01-harness-compatibility.md` (§2). **Amended 2026-09-24: a peer marked `optional: true` in `peerDependenciesMeta` is left out at harvest, and github entries carry the field too — once `SHOP_EMIT_REPO_PEERS` flips.** Until then the definition read "the package's declared `peerDependencies` names", which recorded optional peers as requirements, and only npm entries had it — see the same document, §9.2 and §9.8. Github peers are harvested and recorded from this change, but emitted only after the release commit that first promotes the module-table-refining client to `latest` flips that flag, because an installed shop from before it judges any `peers` by node resolution alone (§9.8).
...
- **Amendment (2026-08-31, market borrowings): a release-rescued entry installs the prebuilt tarball URL.** For an entry carrying `tarball`, the spec is the snapshot's tarball URL — built from snapshot fields, never from the wire — instead of the `github:` form, and the git-on-PATH check does not apply. The Host validated the binding at catalog parse (a coherence check on both the cached and the fresh path): the URL must be an https release of the entry's own `repo`, so a row naming a trusted repo can never install an archive from elsewhere. The recorded tag is the displayed `version`. The sha256 is enforced at install: the Host fetches the tarball and verifies its bytes against the recorded hash before spawning, refusing with a typed `tarball-integrity` rejection on a mismatch or an unverifiable download. The recorded hash, not the URL, is what the audit rests on: a release download URL names a tag and a file name, and an asset deleted and re-uploaded under its old name serves different bytes at the same URL. The check catches that, passive MITM, and asset tampering at the check instant. The install itself re-fetches through pnpm, so an asset swapped between the check and pnpm's fetch remains a TOCTOU window.

  **Amended 2026-09-25:** this bullet said GitHub release assets were immutable per URL. The premise is false: the URL is `/releases/download/<tag>/<file name>`, and nothing stops an author deleting an asset and uploading different bytes under the same name. Read as true, it makes the URL, which the catalog chain pins, proof of the bytes, so the recorded sha256 looks redundant and a reader that re-reads the asset could skip it; the harvest's declarations re-read would then take a replaced asset's `peers` and `compatibility` as the verified tarball's. That re-read believes an asset only once it hashes to the recorded sha256, and treats a mismatch as a changed asset (2026-09-01-harness-compatibility.md, section 9.8; 2026-08-31-market-borrowings.md section 3.1 is amended the same way).

### 7.3 RPC contract

| Method | Arguments | Returns |
|---|---|---|
| `shop/catalog` | `{ refresh?: boolean }` | `{ schemaVersion, builtAt, stale, plugins[], denied[], notAShop?, stars, incompatible, incompatibleHarness }` |
...
**Amendment (2026-09-24): the `shop/catalog` row lists the whole result.** It had named four fields since before `denied`, `notAShop`, `stars` and `incompatible` were added, and this change adds a fifth, so the row now lists all nine rather than only the new one — a row that looks complete is read as complete. `incompatible` is the HOST half of the peer verdict: the client removes every name its live module table serves before anything renders (2026-09-01-harness-compatibility §9.1). `incompatibleHarness` carries each unmet `dsh.compatibility` half with both sides, what the author declared and what is running (§9.9).
...
- **`maintainer:` IS a filter, and it composes with `keywords:`.** "The only filter the API honors" is wrong. Measured live: `keywords:deepseek-harness maintainer:<nonexistent>` returns **0**, not the unfiltered total, and `maintainer:bowenliang123` returns 2 alone and 2 intersected with the keyword. What genuinely is NOT honored — every one of these returns the unfiltered 5,407, i.e. no filtering at all — is `is:unstable`, `not:unstable`, `is:insecure`, `not:insecure`, `is:deprecated`, `not:deprecated` and `scope:`. So the "no negation" half stands, and "keywords is the only filter" does not.
  ...
  **`author:` is honored too, and it is a DIFFERENT axis — 2026-09-09.** This bullet enumerated only what is not honored, and a later code comment read that silence as "`keywords:` and `maintainer:` are the two qualifiers the API actually honours", which is false. Measured: `keywords:dsh-plugin author:sayedev` returns **0** where `keywords:dsh-plugin maintainer:sayedev` returns **1**, while `author:amazing-tree-keeper` and `maintainer:amazing-tree-keeper` both return 2, and `author:nosuchauthorxyz` returns 0. So `author:` filters and discriminates, and it indexes a different field: a package's declared author is not who may publish it, which is exactly why the publisher axis is built on `maintainer:` and not on this. Recorded here rather than in the code because this bullet owns which qualifiers filter; a second copy is how three counts of one figure already drifted apart in this repo.

  **What this does NOT buy, stated because the obvious inference is wrong.** Every package has a maintainer and not every package has a second keyword, so a publisher partition has no *structural* blind spot where a refinement partition does. That argues it should cover more. **Measured, it covers less**: seeded from the window's own 3,041 maintainers, publisher cells recover **95 of the 157** unreachable names (60.5%), against the shipped refinement list's **156 of 157** (99.4%). The structural advantage is real and the discovery constraint dominates it — a cell can only be queried for a maintainer already SEEN, and a single fat refinement (`keywords:deepseek-harness,dsh` alone recovers 123) reaches far more of the tail than the straddling condition a publisher cell needs. Publisher partitioning is therefore a **supplement aimed at a specific failure mode, never a replacement** — *for this keyword*. That qualifier is load-bearing and was missing when this paragraph was written: see *The second keyword's blind spot is measured* below, where no refinement reaches the uncovered set at all, so the same axis carries a load there it does not carry here. Be precise about what that measurement does NOT establish: 99.4% is recovery of names PAST the window, `keywords:dsh-plugin` has none yet, so its counterpart is unmeasured rather than refuted — and at the 4.4% its tail sample gives, refinement recovery would still be ~95.6%, comfortably above {@link MIN_UNREACHABLE_RECOVERY}. What breaks first there is the absolute residual cap, not the recovery rate.
...
**Amendment (2026-09-19): three further probes, and the cheapest structural option is ruled out.** Each qualifier below was compared against a same-instant baseline of `keywords:deepseek-harness` = **7,220**, so a total moving under the harvest cannot be mistaken for a filter that works.

- **No date or range qualifier is honored, which removes the move the GitHub half uses.** `created:>2026-01-01`, `modified:>2026-01-01` and `date:>2026-01-01` each answer the unfiltered 7,220. This is recorded as a negative result because the other half of this project solves the identical problem the other way: `harvestRepos` splits a topic into stars/date/size windows to get under GitHub's 1,000-result cap. There is no npm counterpart, so "partition the keyword by time" is unavailable rather than merely unbuilt. (`not:deprecated` answered 7,220 again, confirming the bullet above; `author:huanlin` answered 5, confirming the 2026-09-09 correction. Neither is new.)

- **`/-/user/<user>/package` is a complete per-owner index and is not the search index**, so the `from` cap does not apply to it: `huanlin` answers **96 packages in one 3,767-byte request**, where `keywords:dsh-plugin maintainer:huanlin` answers 26. It still cannot replace a publisher cell, and the reason is a second measurement rather than a guess — the endpoint is unfiltered by keyword, and the abbreviated packument (`Accept: application/vnd.npm.install-v1+json`, **28,371 bytes** against **43,104** for the same package) **omits `keywords` entirely**. Turning a name from this endpoint into "does it carry a harvest keyword" therefore costs a full packument, which for `huanlin` is 70 wasted fetches to find 26 names one search cell already returns filtered. The endpoint is the right tool only where an owner's cell could itself exceed the window, which no measured account approaches.
...
**Amendment (2026-09-16): the second keyword has about three DAYS left, not three weeks.** Measured live at **5,024** against the same 5,250 window, with the daily CI logs reading 4,666 on 09-11 and 4,966 on 09-15 — about **72 names/day** rather than 60, and 226 of headroom, so it crosses `SEARCH_WINDOW` about **2026-09-19**. The paragraph above is kept rather than corrected in place because what changed is the rate it assumed, not its arithmetic, and because the ten-day error is itself the operative fact: every plan that budgeted against this horizon budgeted against the slower one.

Every consequence the paragraph above draws is unchanged and merely arrives sooner — the free seed of 349 maintainers shrinks at the crossing, and the 81 names carrying no refinement pass out of reach on that date. Two further things the compressed horizon breaks, neither of which is visible from the date alone:

- **The publisher rotation's sizing no longer closes.** `PUBLISHER_PROBE_BUDGET_DEFAULT` is argued from the premise that one rotation cycle must finish well inside this horizon. At 500 against a 3,735-name vocabulary a cycle is ~7.5 runs; that does not fit inside three days. The budget did not move — the horizon did.
- **The residual cap is already spent, twice.** `MAX_UNREACHABLE_RESIDUAL` was raised from 10 to 14 on 2026-09-16 to publish past a `deepseek-harness` residual of 11, which was the ceiling of its bracket, and from 14 to 20 on 2026-09-18 — past that ceiling — to publish past a residual of 17 on the day both harvest keywords crossed the window. The 81 names arrive on top of a cap that has already left the bracket, so the crossing reddens the build on the day it happens rather than after any period of drift, and the next raise has no measured magnitude to appeal to at all.

Both belong to [#38](https://github.com/LivXue/dsh-plugin-shop/issues/38), which is a near-term item on this measurement rather than a late-September one.

*The second keyword's blind spot is measured, and no refinement closes it.* Because `keywords:dsh-plugin` is still fully enumerable, its coverage was measured before the fact rather than after the build went red. `PARTITION_KEYWORDS`' comment owns the figures; the conclusions are:

- The share of names carrying **none** of the 26 non-self refinements is **2.04%** for `keywords:dsh-plugin` (81 of 3,973, the whole keyword) against 0.019% for `keywords:deepseek-harness` (1 of the 5,250 it can address, a window that excludes the tail). The two populations differ, so the ratio between them is not a measurement — `PARTITION_KEYWORDS`' comment says which pairings are and are not licensed. The magnitude that matters needs no ratio: two percent is enough to breach the residual cap and a fifth of one percent is not. The cause is a tag habit, not a coverage accident: `deepseek-harness` is almost always co-published with `dsh` or `dsh-plugin`, while `dsh-plugin` is the conventional plugin tag and is frequently the only one a package carries.
- The rate is worse at the bottom of the ranking, which is what a tail is made of: 4.4% over the lowest 250 ranks. **Do not multiply that into a date** — that is the drift model this section refutes two paragraphs above, and it over-predicted fivefold on the keyword where both its inputs were measured. The step reading applies instead, and it is the harsher one: 38 owners hold those 81 names and one of them holds 21, twice the cap, so a single family landing past the window together reddens the build on the day it lands. What the rate bounds is how fast uncovered names enter the tail, and that is what makes the unit **days rather than months** — an ordering estimate of two to seven puts it within a week of the crossing. Note also which guard fires: at 4.4% refinement recovery is still 95.6%, above {@link MIN_UNREACHABLE_RECOVERY}, so it is the absolute residual cap that breaks, not the rate floor.
...
- The publisher axis reaches **all** of them: every one of the 81 carries at least one maintainer username that Task 1 of the plan's grammar accepts, so a cell can be built for each. That is reachability, not a coverage win — the 81 are *defined* as the names no refinement reaches, so "81 of 81 unreachable by refinement" restates the definition, and over the shared population (this keyword's 3,973 names) the two axes are 97.96% against 100%, two points apart. The informative comparison needs a set defined by neither axis, which is what the 157 past-window names gave the 60.5%-vs-99.4% pair; `dsh-plugin` has no such set until it crosses. What this does establish is the asymmetry the bullet above needs: refinements have no move here and publisher cells have one. Concentration describes that set — 38 distinct maintainers own the 81, the seven largest cells reach 51 (63%), a minimum cover is 37 cells — and it is **not** the run cost. The 37 are oracle-selected; the mechanism probes one cell per known publisher, so the price is the vocabulary (~3,390 once this keyword is seeded) plus an unbounded paging half, which the plan's Task 5 sizes at roughly tripling the npm half of a run. The username must be read off `maintainers[].username` — `publisher.username` is the last publishing identity and can be a CI bot's display name (`GitHub Actions` for 12 of these), which `maintainer:` cannot address.

So the ordering in this section stands — publisher cells recover less of the *first* keyword's tail than refinements do — but the conclusion drawn from it does not generalise. On the second keyword refinements have no move against the uncovered set at all, so the publisher axis is the only one of the two that can act there.

**That is not the same as saying it will work there, and the difference is this section's own discovery constraint.** All 81 are inside the window today, which is exactly why every one of their maintainers is SEEN — the property the reachability result depends on is a consequence of the condition that ends at the crossing. The paragraph above already states the pressure: "when it crosses, that free seed shrinks — at the moment it is most needed." The only measurement of publisher-cell recovery over a real past-window tail remains 60.5%, and nothing here revises it. A publisher whose whole family lands past the window is as invisible to `maintainer:` as to `keywords:a,b`, because no query-based axis is covering — the `from` cap is the wall, and both axes are queries. So the plan in `docs/plans/2026-09-08-publisher-partition.md` acquires a date it did not have when it was written; it does not acquire a guarantee, and the covering route is still the replication feed priced above.
...
A bound sized for the first cannot hold the second, and a bound sized for the second hides a partition collapse — precisely what the existing comment refuses to allow. **So the two are separated before any partition work is done**, because every option in the amendment above needs the distinction and none of them supplies it. The harvest computes `max(0, required − SEARCH_WINDOW)` from the minimum of the probed totals, reports recovery against this single-window tail, and requires both a recovery rate of at least 90% and a residual of at most 20 names for gaps larger than the noise allowance. Every tolerated shortfall is reported.

The residual cap was bracketed, not chosen: **at least 9**, the headroom a publisher family needs, which must be absorbed rather than redden the build; and **under 15**, the size of the one real partition gap this repo has measured, which must never be absorbed. **Amended 2026-09-18: the upper bound is abandoned.** At 20 the cap absorbs that 15-name gap silently, and nothing else refuses one — `MIN_UNREACHABLE_RECOVERY` is a rate, and from a 150-name tail up a gap that size already clears it. The floor still holds. See `MAX_UNREACHABLE_RESIDUAL`'s comment for what forced it and what it buys.

**Amendment (2026-09-16): the cap moved from 10 to 14, and the premise that it was sized against no longer describes the live shape.** The cap was set on the reading that the residual "has held at exactly 1 across every reading since this keyword crossed the window" — a step function driven only by publisher families arriving together. Six consecutive daily runs of `keywords:deepseek-harness` refute that: 2, 19, 23, 6, 8, 11, where the 6 follows `29b941d` paging an oversized refinement cell and the 11 reddened build [35074642213](https://github.com/LivXue/dsh-plugin-shop/actions/runs/35074642213). Between those two readings the residual rose about 2.5 a day against a tail growing about 77 a day, which is drift: a few percent of arriving tail names carry no refinement any cell reaches. The step function is real and still present; it is not the only term.

14 is the ceiling of the bracket above, taken to publish a catalog that was otherwise a day stale, and it is spent rather than restored — there is no room left, because 15 is the partition gap the ceiling exists to refuse. The property the original comment states, that crossing this cap means the PARTITION has to improve, is unchanged and now overdue; the answer is [#38](https://github.com/LivXue/dsh-plugin-shop/issues/38) and not a larger number here.
...
**Amendment (2026-09-09, publisher axis BUILT): `maintainer:` cells supplement the refinement list, seeded from every page the harvest reads and persisted across runs.**

A `keywords:<harvest>,<refinement>` cell cannot reach a package carrying only the harvest keyword; `keywords:<harvest> maintainer:<user>` has no such blind spot, because every package has a maintainer. It has a different limitation — a cell can only be built for a publisher already SEEN — so it is a supplement, and the measurement above says so: window-seeded publisher cells recover 95 of 157 where the refinement list recovers 156 of 157 (2026-09-08). What they uniquely reach is the family event: one publisher releasing together, ranking together at the bottom, sharing no tag. The `sayedev` incident was exactly that, and 14 of its 20 packages were already visible, so the cell was derivable from what the harvest had already read.

The vocabulary is free (search responses carry `maintainers`), collected from EVERY cell rather than only the over-window one, and persisted in `registry/publisher-state.json` — a committed, sorted, deterministic build input in the shape of `repo-state.json`, monotonic because the window that discovers a publisher is a shrinking fraction of the keyword. CI carries observed usernames in the classifier's `dist/harvest.json`; the build merges them with prior state after either its direct harvest or the handoff branch, and the snapshot commit stages the vocabulary after the build writes it. So the next run receives the publishers the previous one discovered, even though CI never executes the build's direct npm search.

**Two costs are bounded separately, and conflating them is the mistake this amendment exists to record.** `PUBLISHER_PROBE_BUDGET_DEFAULT` bounds the PROBES — one `size=1` request per vocabulary entry. It does not bound the PAGING, and nothing about a non-zero probe suggests a cell is worth paging: every maintainer in the vocabulary was read out of a `keywords:<harvest>` result, so that keyword's cell for them is non-zero by construction. Paging every non-zero cell measured at ~3,041 cells, twice — the retry re-pages the whole partition, and for `keywords:deepseek-harness` the residual sends it round again on every run — against a baseline of ~6,100 npm requests: ~2.5x today, ~4x once `dsh-plugin` crosses. So a cell is paged only when `cellTotal` exceeds the DISTINCT names already served for that maintainer by the window sweep and the refinement cells. The excess has to be somewhere nothing has looked, which past the window is where this axis aims. A count of occurrences rather than distinct names inflates as cells re-serve the same names and would skip a cell that really does hold something new.

**The budget is a window on the vocabulary, not a prefix of it.** A fixed start probes the same first N names on every run and the rest on no run at all — deterministic starvation, and silent, because a publisher who is never probed cannot be reported missing. `PublisherState.cursor` carries the start position across runs and advances by one budget per successful build, wrapping, so a full vocabulary is covered in `vocabulary / budget` runs — the same accumulate-over-days posture as the vocabulary itself. It advances only on a run that reached its artifact block, so a failing run does not skip its slice.

**The budget must therefore stay BELOW the committed vocabulary.** `nextCursor` returns 0 while the whole vocabulary fits one run, so a budget sized above it disables the rotation outright and every run probes everything in one sequential pass. That is not a hypothetical: the axis shipped in 0.8.1 with a 4,000 budget against a vocabulary the same release committed empty, so CI only ever exercised the no-op path; the first green run wrote 3,474 publishers and the next run spent 57m37s probing them before npm answered 503 and the build published nothing (2026-09-10). The budget is now bracketed against three measured magnitudes — the committed vocabulary, the wall-clock cost of a probe, and the date `keywords:dsh-plugin` crosses the window — in `PUBLISHER_PROBE_BUDGET_DEFAULT`'s comment, which owns the value; this section deliberately does not restate it.

**A partial run recovers only from the slice it probed.** The vocabulary and the cursor persist across runs; the probe RESULTS do not. So the rotation guarantees every publisher is eventually probed and never that one run sees all of them — the difference from `REPO_BACKFILL_BUDGET`, which persists its per-repository outcomes and does accumulate in that stronger sense.

**A publisher cell's own query is the one place a maintainer reaches npm, so `cellQuery` throws rather than drops on a username outside the grammar.** `text=` is a query language, not a value: `encodeURIComponent` turns `x keywords:evil` into a valid URL and npm reads three qualifiers where the caller wrote two, enumerating a set the cell's probed `total` was never measured against. Every producer validates (`maintainersOf` on the way in, `parsePublisherState` on the way off disk), which is why arriving there unvalidated means this project's own plumbing lost the invariant. The same rule holds at both ends of the persisted file: a writer that does not apply the grammar the reader throws on can commit a file every subsequent build refuses to parse — a self-inflicted stop one run away from its cause, repairable only by hand-editing a generated file.


**Amendment (2026-09-14, oversized-cell split): an over-window refinement cell is replaced by intersections that cannot cover it, and the substitution is provably empty in exactly the case that arrived first.**

`partitionKeyword` probes each refinement and sorts the answer two ways: a cell inside `SEARCH_WINDOW` joins `cells` and is paged, a cell past it goes to `oversized` and is replaced by the deeper intersections `keywords:<harvest>,<over>,<other>` for every other refinement that splits it. That replacement was reasoned about for the case where two fat refinements straddle each other, and it is correct there. It is **empty** in the case the ecosystem actually produced.

*Why it is empty, before any measurement.* `<over> ∩ <other> ⊆ <other>`. Every `<other>` is itself a refinement cell, and while it fits the window `enumerate` pages it in full. So each intersection the split produces is a subset of a set the same run has already enumerated, and the split contributes nothing that paging the plain cells did not. The substitution buys coverage only when the deeper refinement is ALSO over the window — the straddling case — and it is worth naming that the common case is the opposite one: the first refinement to cross the window is the fattest, and the fattest is the one the others are all smaller than.

*What it cost.* `keywords:deepseek-harness,dsh` crossed `SEARCH_WINDOW` on 2026-09-11, between the 08:03 scheduled run — which published `2026.911.1` — and the 21:54 run, which did not. Every catalog build since has been red on the residual cap and the published catalog has not moved. The two runs bracket the crossing by their residuals (2, then 19); the cell measured 5,449 on 2026-09-14, which the keyword's own growth rate puts within tens of names of the window on the 11th.

*Measured 2026-09-14, by running both partitions live against the registry.*

| partition | union | residual against the answered total |
| --- | --- | --- |
| the shipped algorithm — oversized cell split into intersections | 6,407 | **34** |
| the same run, oversized cell paged to the window instead | 6,431 | **10** |

The shipped union is a **strict subset** of the other — zero names it holds that direct paging does not — and that, rather than the set-theory argument above, is the measurement: the split is not merely weaker, it contributes nothing at all. None of the deeper `dsh,<other>` cells supplied a single name in either run.

The 24-name difference all carry `deepseek-harness` and `dsh` and **no other refinement** — `dsh-code-navigator`, `dsh-plugin-session-export`, `dsh-plugin-vault-memory`, `dsh-adblock`, `dsh-viewer`, `@young1lin/dsh-cc-otel`, `@quan-huayan/dsh-research-engine` and seventeen more. They are precisely the set a refinement partition is defined to miss, and the oversized cell was the only thing that had been reaching them.

*A hypothesis this displaced, recorded so the next reader does not spend the same hours on it.* The publisher rotation moved from `cursor: 2500` to `cursor: 3000` in the 08:03 run — the same run whose successor first went red — and `tudamu`, who holds 218 packages under this keyword and 45 of the window's bottom 250, sits at vocabulary index 2,824: inside the slice the passing run probed and outside the slice every failing run has probed since. The timing is exact and the causation is absent. All 218 of that publisher's packages carry `plugin`, `dsh`, `agent`, `cordis` and `deepseek`, so refinement cells reach them whatever the cursor does. The publishers who DO own the 24 lost names sit at indices 83 through 2,514, none of them in the probed slice, and the slice that preceded it covered exactly one of them — the axis was not what had been reaching these names on 2026-09-11, and the oversized cell was.

*One latent defect found while measuring, which is NOT this failure but would perpetuate a different one.* `build.ts` advances the cursor after the artifact block, deliberately — "the cursor advances only on a run that got this far, so a failing run does not skip its slice of the vocabulary." The coverage check throws in `classify.ts`, long before that line, so a red build commits no snapshot and the cursor does not move: it stood at 3,000 from 2026-09-11T08:53 until the fix below reopened the catalog on 2026-09-14T09:52, and the rotation was stopped for the whole outage — three days in which no publisher was probed that the frozen slice did not name. It resumed with that run, to 3,500. The intent is right and the consequence is that **a run failing BECAUSE of its slice can never rotate out of that slice** — a self-perpetuating red the rotation was supposed to make impossible. Nothing in the present failure depends on the slice, so this is latent; it needs an advance that survives a failed coverage check, not a change to where the cursor is written.

*Options, measured 2026-09-14 against the live registry.* None is free, the first is not a fix on its own, and the first two compose in a way that is easy to miss.

1. **Page the oversized cell to the window.** `pageCell` already takes a `pastWindow` mode and the window sweep already uses `'stop'` for exactly this reason; an oversized refinement cell would take the same mode and let the coverage arithmetic account for what the cell cannot reach, as it already does for the keyword itself. Measured two ways, and the gap between them IS option 2's contribution: the partition alone — window sweep plus refinement cells, no publisher axis — leaves **10 against a cap of 10**, while the shipped harvest run end to end against the live registry, with the committed vocabulary and the 500-probe slice its cursor names, clears it. **The reading that counts is the publishing run's, and it is 6**: `2026.914.0` enumerated 6,442 of an answered 6,448, recovering 1,192 of the 1,198 past the window. Three readings inside two hours give 5 (local, total 6,442), 8 (the PR dry run, total 6,445) and 6 (the publishing run, total 6,448) — the enumeration tracks the total, and what moves between readings is npm's count, not the partition. So the residual is a **5-to-8 band rather than a number**, and the margin against the cap is **two to five names**. Quote the band; a single figure from one run will mislead whoever checks it next. What option 1 structurally cannot reach is the oversized cell's own 198-name overflow: it is bounded by that cell's window, which is the same wall one level down.
2. **Reach them with the publisher axis** — and NOT, as an earlier draft of this amendment said, by building publisher cells ON the oversized cell. That shape is redundant and the redundancy is structural: `keywords:A,B maintainer:M` is a subset of `keywords:A maintainer:M`, so a cell narrowed by the oversized refinement can never reach a name the top-level cell for the same maintainer does not. Measured over eight owners of the lost names, the top-level cell answered greater than or equal in every case and strictly greater in one (`moqian`: 9 against 6). The axis therefore needs no new cell shape, and what binds it is the budget: **21 of the 24 owners are in the committed vocabulary**, three are not (`leaf233`, `lifangjin`, `sujiu522`), and exactly **one** is inside the 500-name slice the rotation is probing. A full-vocabulary sweep would reach 21 of 24 and cost ~3,541 sequential probes — the shape that produced the 503 on 2026-09-10 and the reason the budget exists. So this option is real, it is not free, and it is not a cell-shape change.
3. **Add a refinement that splits `dsh`.** Available here, and that is the correction: unlike `keywords:dsh-plugin`, whose uncovered set carries no third tag at all, every one of these 24 carries one. They are simply heterogeneous — a greedy minimum cover needs **15 keywords for 24 names** (`concept`, `celestea`, `seccomp`, `typescript`, `定时发送` among them), the largest single one reaching 4. So this is not the structural impossibility recorded for the second keyword; it is the per-incident bill `PARTITION_KEYWORDS`' comment already refuses, priced: fifteen entries bought for one day's names, each one a probe on every future run, and each a one-off tag with no reason to recur.
4. **Drop `dsh` from `PARTITION_KEYWORDS`.** Coverage is **identical** — both partitions measured 6,407 — because the deeper cells contribute nothing, so the entry currently buys one probe, twenty-five deeper probes and their paging for zero names. Not a fix; the accounting. It says the entry is already inert while reading as load-bearing, which is what let the regression arrive silently.

**Options 1 and 2 compose, and the composition is worth more than either.** Option 2's three misses are its discovery constraint — an owner enters the vocabulary only from a page the harvest read, and these three publish only past the window, so nothing has ever seen them. Option 1 pages the oversized cell and therefore **sees all 24**, which puts those three owners into the vocabulary for the run after. So option 1 is not merely a reprieve: it is the mechanism that feeds option 2 the seed option 2 cannot obtain for itself, and option 2 is what carries the names option 1's own window will lose next. Neither works alone — option 1 runs out with the cell's overflow, option 2 starts three names short and stays there.

**Option 1 is implemented; option 2 is not.** The margin it leaves is measured rather than assumed, and it is **four names** on the run that reopened the catalog — inside a band whose worst reading that day left two. Of the oversized cell's own bottom 250 names, **0.8%** carry `dsh` and no other refinement, and that cell's overflow grows with the keyword, so the residual should climb on the order of half a name a day. Three readings inside two hours do not establish a rate, and the run-to-run noise is larger than a day of drift, so the honest statement is that the next decision has days to a couple of weeks and that the first sign will be a red build rather than a trend anyone watched. An earlier draft of this paragraph said a week or two from a partition-only figure taken before the publisher slice had run, then two names from the dry run alone; both are recorded rather than overwritten, because each is what a reader measuring that one way would reproduce.

**One prediction this amendment made was confirmed by the run that reopened the catalog.** Option 1 was argued above to feed option 2 the seed option 2 cannot obtain for itself, because paging the oversized cell exposes owners who publish only past the window. The publishing run committed `publisher vocabulary 3541 -> 3685` — **144 usernames the frozen vocabulary had never seen**, among them the publishers option 2 could not previously have been queried for at all. The composition is not theoretical, and the figure is the committed one: the PR dry run computed 3,682 and wrote nothing, which is the number to avoid quoting.

The covering route is unchanged and still priced in the 2026-09-08 follow-up: no query-based axis is covering, because the `from` cap is the wall and every axis here is a query — including, as this amendment shows, an axis applied one level down.
...
**A github entry gets no size, deliberately.** (Superseded 2026-09-08 by `installSize`, which measures the git tree's blobs rather than asking GitHub, and rendered by the client from the release that carries the 2026-09-15 follow-up below — a schema change, so it goes through `beta` first and the promotion commit is what pins the number. What stands is the reasoning: the figure GitHub REPORTS is still refused, and `unpackedSize` still admits no github entry.) GitHub's repo `size` is the repository's own disk usage including history, which is not the plugin — and for a monorepo subpackage it is not close. The rescued-release tarball's size is the COMPRESSED artifact, which is a different quantity from npm's unpacked figure and would put two incomparable numbers under one label. A number that measures something other than what its label says is the class of plausible-and-wrong this project stops for, so those entries print nothing. The client's `formatSize` is `undefined`-in/`undefined`-out for exactly that.

The label is decimal (`kB`/`MB`/`GB`, not `KiB`), because the figure IS npm's own and npmjs.com shows it decimal — a reader checking the shelf against the package page must not find two different numbers. It is locale-free by construction (`toFixed`, never `toLocaleString`): a decimal comma in one language reads as a thousands separator in the other. The visible text is the bare figure; the accessible name and tooltip say WHICH size it is, since an on-disk figure and a download differ by a ratio that is itself unstable. That word was **unpacked** until the release carrying the 2026-09-15 follow-up and is now **on disk** / **磁盘占用**: a commit-pinned github entry unpacks nothing, so npm's word could not cover both sources once `installSize` rendered.

*Card layout.* The size sits at the right end of the action row immediately LEFT of the author, both inside one wrapper that owns the `margin-left: auto`. Note what that ordering costs: the author's width varies and is absent entirely on many entries, so a size's horizontal position moves from row to row and the figures do not form a column. The size is mono and `tabular-nums` for per-figure legibility, which is the honest reason; an earlier draft of this amendment justified the monospace by a column this layout cannot produce. The wrapper is what makes the group stay flush right when only one of the two has a value. (Until `installSize` rendered, that was the COMMON case, because size was absent on every github entry; it is now the rare one, and the wrapper earns its keep for a different reason — without it an EMPTY meta group still costs 8px, since `.cardActions` is a flex row with `gap: 8px` and a zero-width item is still an item.) What fixes the ORDER is that the author's width varies far more than a formatted figure's, so the outer edge is the one that must absorb the variance.
...
**Amendment (2026-09-15): that client follow-up, and what its absence cost.** It went undone for a week. 0.8.1 and 0.8.2 both shipped while the registry published an `installSize` for every entry — the 2026-09-14 build report reads "No install size: 0 of 10767 (github 0, npm 0)" — and every client stripped it, so all 6,492 github cards showed no size at all. The consumer schema is non-strict by design, which is the property that let the registry ship first; it is also the property that makes a consumer never written look exactly like a consumer working. Nothing failed, no report counted it, and `git log -S installSize -- packages/dsh-plugin-shop` was empty.

*What that argues for.* An additive key's producer and consumer are one change that may be SEQUENCED, not two changes. The registry half of a new key should land naming the client release that will read it, and the field is not done until a test asserts the value through a real host parse. That last clause is the operative one here: the jsdom specs build their snapshot object directly, so they would have passed against a schema that still stripped the key. Only `web-full-flow.e2e.ts` crosses the real zod, and it asserted a size arriving under `unpackedSize` — a key that already worked. It now carries both lanes: `dsh-e2e-fixture-plugin` holds the old key alone (the compatibility path the parse-boundary merge below exists for) and `dsh-shop-e2e-live` the new key alone. Be precise about what that second lane proves, because the first draft of this paragraph overstated it: `dsh-shop-e2e-live` declares no `source`, and `source` defaults to `npm`, so it parses down the npm arm and is NOT "the shape of a github entry". It cannot be one — the same name is installed from the local npm registry by the hot-mount spec, and the github arm demands a `repo` and a commit-sha or release-tag `version`. What it proves is the property that actually broke: a key undeclared yesterday survives a real host parse and reaches the card. The github arm carrying an `installSize` is pinned by `catalog.test.ts`, which crosses the same zod without a browser.

*The two size keys merge at the PARSE boundary, so the client reads one field.* `entrySchema` ends in a `transform` that fills `installSize` from `unpackedSize` when only the older key is present, and `ShopTab` reads `formatSize(entry.installSize)`. The keys hold one quantity — measured 2026-09-15 over the live catalog, all 4,275 npm entries carry both and they are equal on every one — so the merge moves no figure today. What it buys is a rolled-back or cached catalog: reading only the new key would blank every npm size the moment the shop met a build predating it.

Why the parse and not the render site, which is where this first landed: the parse is the only layer that knows an entry's `source`. The `superRefine` immediately above the transform has just PROVEN that an `unpackedSize` present at all belongs to an npm entry, where the two keys name one quantity — so the merge is provably source-correct there, whereas at the render site it was correct only by inheriting an invariant declared ninety lines away and invisible from the call. It also makes the precedence a property of a parsed `CatalogEntry` rather than of one JSX expression, so a second consumer (a sort, a detail row, an install confirmation) cannot read the stale key by accident, and retiring `unpackedSize` becomes a deletion here instead of a hunt for every read site. `CatalogEntry.installSize` is THE size field; reading `unpackedSize` in client code is a bug.

*A second key had the same defect, and it was still live.* `review.repo` — which repository a github review was bound to — is written by `config.ts` onto every stored Review, carried through `tier.ts` and `emit.ts` verbatim, and was declared by no consumer, so every client stripped it exactly as it stripped `installSize`. It is dormant only because `verified.yml` is empty, which is the state in which nobody looks; it matters precisely where this project says identity matters most, since a github review binds (`repo`, `reviewedCommit`) rather than a name because 83 live bundle names are claimed by both a fork and an original. Declared now, while the cost is zero.

*So the rule gets a mechanism, not a convention.* Both keys were written by people who knew the rule, so "remember to declare it on both sides" would have caught neither. `registry/scripts/tests/catalog-parity.test.ts` reads the registry's published `Entry` and `Review` declarations and the consumer's `entrySchema`, and fails on a key the registry publishes that the consumer does not declare. It lives on the producer side so the PR which ADDS a key is the PR that goes red, and its exemption table is empty — it was empty the day it was written, which is what makes it a gate rather than a backlog. It also fails loudly when its OWN parse stops matching, because a structural guard that silently parses nothing passes forever while covering nothing.

*The label changed for both sources, the number for neither.* See the 2026-09-07 paragraph above, corrected in place.

*What this change deliberately does NOT carry.* The README feature rows and the six generated screenshots still describe the release they pin, which does not render a github size. Both move in the promotion commit that advances the pin — the screenshots because they shoot the PUBLISHED build and a partial reshoot would put two catalogs side by side in one table, the README row because its prose tracks `latest`. The row to add says what is measured: the git tree's blobs scoped to `subdir`, or the inflated release archive — never the size GitHub reports for the repository, which this document refuses two paragraphs up.
...
**Amendment (2026-08-31, market borrowings C-1): restart under a supervisor is refused by default.** *(Superseded in part 2026-09-14: the refusal is unchanged, but `shop/version` now answers `restartBlocked: 'systemd'` rather than a boolean — see §14.)* When a systemd unit owns this process and the shop row config sets no `allowRestart: true`, `shop/restart` issues a typed refusal and the two-phase handoff never starts — the main process exiting also kills the unit's cgroup, which takes the detached helper with it, and the service would never come back. `shop/version` reports `restartSupported: false` and the client hides the restart offer while keeping the pending-change notice (§7.3).
...
| Pointer names a data file the cache cannot address — `.`, `..`, a path, or a separator in either spelling | Host-side and loud, **before the fetch**: the shop throws, the client shows its generic error state | **Fail loudly.** The url is untrusted, and `basename()` — the derivation this replaced — is on such input neither confining nor platform-independent: `basename('.')` made `join(cacheDir, …)` collapse to the cacheDir ITSELF, so a real `writeFileSync` was handed a directory and answered EISDIR out of the loader, and a backslash reads as a separator on Windows but as a legal filename character on POSIX, so one pointer named two different cache files depending on the host. A plain file name is tested rather than derived. `npmOrigin.file()` has always refused these outright, so this is the HTTP transport adopting the npm transport's rule — without it the two disagreed about the same pointer, which §2 does not allow. The stars sidecar is the sole exception, as everywhere in §5: the identical refusal costs the stars and nothing else |
| Disk cache cannot be written — read-only cacheDir, full disk, another dsh holding `index.json` open | Nothing. The fetched, verified snapshot is served as if the cache had been written | **Degrade, and commit in dependency order**: the data file first, then the pointer that names it, then the freshness note that declares that pointer current — a file becomes reachable only once whatever it names is already on disk. Reversed, a failure after `index.json` landed left the pointer naming a file that was never written and `readCached` read the whole cache as ABSENT, so the failed write did not merely fail to cache the new catalog, it unbound the good one already there and the next offline boot threw instead of degrading. And the write is not a load failure: the bytes in hand are already verified, so throwing would take a working catalog down over an optimisation |
...
- **`shop/restart` is POSIX-only.** The two-phase handoff runs a shell one-liner through `sh` with `kill -0` and `sleep`, none of which Windows has, and `exec "$@"` has no Windows equivalent — it needs a detached node helper instead. Until then the Host refuses the call before anything is torn down and reports `restartBlocked: 'windows'`, so the client keeps the pending-change notice and drops the offer. This is a refusal rather than a deferral because the failing spawn is asynchronous: committing would answer `ok: true`, exit the process, and leave nothing holding the port.

  **Amendment (2026-09-14): a boolean could not carry the reason, and routing Windows down "the same path as the systemd case" is what made the shop lie to every Windows reader.** This clause used to end "(the same path as the systemd case)", and the client implemented it literally: `version()` answered a boolean, and the client had ONE string for its false — the systemd one. A Windows reader was therefore told that dsh runs as a systemd service and to set `allowRestart: true`, an override the platform check — first of the gates, and the only one with none — could never reach.

  `version()` now reports `restartBlocked: 'windows' | 'systemd' | 'port-zero' | null`, computed by the same ordered predicate `restart()` refuses with, and the client renders that reason's own copy. Three consequences worth stating, because each was a live defect:

  1. **`--port 0` is now advertised, not only enforced.** `restart()` has always refused it — the OS hands the new process a different port and the browser is stranded — but the boolean was computed from the platform and the supervisor alone, so such a host displayed a restart button that was guaranteed to refuse. This project's own web e2e boots exactly that way, and asserted that dead affordance was correct.
  2. **The order is part of the contract.** Windows precedes systemd precisely because the systemd copy offers an override; naming the overridable reason first would send a Windows reader to a config key that leaves them where they were.
  3. **A running install stays out of it.** `restart()` refuses that too, but it is transient, and `version()` is read once at mount: a card would go on saying a restart was impossible after it had become possible. Static reasons are advertised; the dynamic one is raised at the call.
