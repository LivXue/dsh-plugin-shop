...
*One latent defect found while measuring, which is NOT this failure but would perpetuate a different one.* `build.ts` advances the cursor after the artifact block, deliberately — "the cursor advances only on a run that got this far, so a failing run does not skip its slice of the vocabulary." The coverage check throws in `classify.ts`, long before that line, so a red build commits no snapshot and the cursor does not move: it has been frozen at 3,000 since 2026-09-11T08:53 and the rotation has been stopped for the whole outage. The intent is right and the consequence is that **a run failing BECAUSE of its slice can never rotate out of that slice** — a self-perpetuating red the rotation was supposed to make impossible. Nothing in the present failure depends on the slice, so this is latent; it needs an advance that survives a failed coverage check, not a change to where the cursor is written.
...
1. **Page the oversized cell to the window.** `pageCell` already takes a `pastWindow` mode and the window sweep already uses `'stop'` for exactly this reason; an oversized refinement cell would take the same mode and let the coverage arithmetic account for what the cell cannot reach, as it already does for the keyword itself. Measured two ways, and the gap between them IS option 2's contribution: the partition alone — window sweep plus refinement cells, no publisher axis — leaves **10 against a cap of 10**, while the shipped harvest run end to end against the live registry, with the committed vocabulary and the 500-probe slice its cursor names, clears it. **The gate's own reading is the one that counts, and it is 8**: CI enumerated 6,437 of an answered 6,445, recovering 1,187 of the 1,195 past the window. A local run an hour earlier read 5 against a total of 6,442 — the same 6,437 enumerated, three more names answered — so the two readings differ by the total moving under them and not by the partition behaving differently. Take the margin as **two names**, not five. What option 1 structurally cannot reach is the oversized cell's own 198-name overflow: it is bounded by that cell's window, which is the same wall one level down.
...
**Option 1 is implemented; option 2 is not.** The margin it leaves is measured rather than assumed, and it is **two names**: the gate read 8 of a cap of 10 on the run that reopened the catalog. Of the oversized cell's own bottom 250 names, **0.8%** carry `dsh` and no other refinement, and that cell's overflow grows with the keyword, so the residual should climb on the order of half a name a day — but two readings an hour apart do not establish a rate, and the honest statement is that the next decision has days rather than weeks. An earlier draft of this paragraph said a week or two, from a partition-only figure taken before the publisher slice had run; the correction is recorded rather than overwritten because the optimistic reading is the one a future reader would otherwise reproduce.

**One prediction this amendment made was confirmed by the run that closed it.** Option 1 was argued above to feed option 2 the seed option 2 cannot obtain for itself, because paging the oversized cell exposes owners who publish only past the window. The first green build reports `publisher vocabulary 3541 -> 3682` — 141 usernames the frozen vocabulary had never seen, of which some are exactly the publishers whose names option 2 could not previously have been queried for. The composition is not theoretical.
