...
*What that argues for.* An additive key's producer and consumer are one change that may be SEQUENCED, not two changes. The registry half of a new key should land naming the client release that will read it, and the field is not done until a test asserts the value through a real host parse. That last clause is the operative one here: the jsdom specs build their snapshot object directly, so they would have passed against a schema that still stripped the key. Only `web-full-flow.e2e.ts` crosses the real zod, and it asserted a size arriving under `unpackedSize` — a key that already worked. It now carries both lanes: `dsh-e2e-fixture-plugin` holds the old key alone (the rollback path the client's `installSize ?? unpackedSize` fallback exists for) and `dsh-shop-e2e-live` the new key alone (the shape of every github entry).

*The client reads `installSize ?? unpackedSize`, not `installSize` alone.* The two hold one quantity — measured 2026-09-15 over the live catalog, all 4,275 npm entries carry both and they are equal on every one — so the fallback moves no figure today. What it buys is a rolled-back or cached catalog: reading only the new key would blank every npm size the moment the shop met a build predating it, which is a regression the rollback was supposed to avoid.
