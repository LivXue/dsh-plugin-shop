...
- **The residual cap is already spent.** `MAX_UNREACHABLE_RESIDUAL` was raised from 10 to 14 on 2026-09-16 (amendment above) to publish past a `deepseek-harness` residual of 11, which is the ceiling of its bracket. The 81 names arrive on top of a cap with nothing left above it, so the crossing reddens the build on the day it happens rather than after any period of drift.
