...
| nature | noise; a re-page may close it | **provable arithmetic**; no re-page ever closes it |
| evidence | seen live: a window paged 5,247 of 5,250 mid-run | seen live: `enumerated 5406 of 5407` |

A bound sized for the first cannot hold the second, and a bound sized for the second hides a partition collapse — precisely what the existing comment refuses to allow. **So the two are separated before any partition work is done**, because every option in the amendment above needs the distinction and none of them supplies it. The harvest computes `max(0, total − SEARCH_WINDOW)` — which it can PROVE, since `total` is the one honest thing the API says about what it will not serve — reports the recovery against it, and fails on a collapsed recovery RATE rather than on an absolute residual. A partition that breaks takes the rate to near zero and still fails loudly; the API's own limit leaves a small residual with the rate high, and that is reported by count rather than pretended away.
