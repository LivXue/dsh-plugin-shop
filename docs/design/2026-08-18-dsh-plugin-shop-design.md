...
**The budget is a window on the vocabulary, not a prefix of it.** A fixed start probes the same first N names on every run and the rest on no run at all — deterministic starvation, and silent, because a publisher who is never probed cannot be reported missing. `MAX_PUBLISHERS` (20,000) against the default budget (4,000) makes that five-sixths of a full vocabulary. `PublisherState.cursor` carries the start position across runs and advances by one budget per successful build, wrapping, so a full vocabulary is covered in five runs — the same accumulate-over-days posture as the vocabulary itself. It advances only on a run that reached its artifact block, so a failing run does not skip its slice.
