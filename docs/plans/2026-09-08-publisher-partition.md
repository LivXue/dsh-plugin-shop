...
This axis can, and it reaches **all 81**: every one carries at least one maintainer username inside Task 1's grammar, so 0 of 81 are unreachable against 81 of 81 by refinement. Concentration decides only the cost — **38** distinct maintainers, the seven largest cells reaching 51 (63%), a full cover 37 cells. `huanlin` alone owns 21, and `keywords:dsh-plugin maintainer:huanlin` probes to 26. So for the second keyword this plan is not insurance against an occasional burst — it is the only mechanism that reaches a residue arriving on a schedule.
