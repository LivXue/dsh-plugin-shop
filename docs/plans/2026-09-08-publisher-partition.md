...
`keywords:dsh-plugin` is still fully enumerable — 3,973 against a 5,250 window, growing ~55/day, so it crosses in late September or early October 2026. Because it is enumerable *today*, its coverage could be measured cheaply before the fact, and it is much worse than the first keyword's:
...
At 4.4% the residual reaches `MAX_UNREACHABLE_RESIDUAL` at a tail of ~227 names, about four days past the crossing; the sample's 95% interval (1.9%–6.9%) puts it at three to ten days. **A refinement cannot close it** — those 81 packages carry `dsh-plugin` and mostly nothing else, so there is no second tag to intersect on.
