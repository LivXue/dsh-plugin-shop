...
This axis can: the 81 names have only **31 distinct publishers**, and seven own 57 of them (70%). `huanlin` alone owns 21, and `keywords:dsh-plugin maintainer:huanlin` probes to 26. So for the second keyword this plan is not insurance against an occasional burst — it is the only mechanism that reaches a residue arriving on a schedule.
