...
**Base:** `main` at or after `4df49bb` (PR #26, merged 2026-09-08), plus PR #27, which lowered `MAX_UNREACHABLE_RESIDUAL` from 25 to **10** and bracketed it at `9 <= cap < 15`. **The gate this section used to carry is lifted**; what follows is what those two PRs left behind, because Task 5's fixtures are written against it. Size any new fixture against 10, not 25.
...
- `describeShortfall` and `parseKeywordShortfall` are exported from `npm-client.ts`, and the handoff between them is ONE-DIRECTIONAL: `classify.ts` imports `describeShortfall` and WRITES the records (`JSON.stringify({ candidates, rejections, shortfalls })`), `build.ts` imports both and PARSES them back. Do not look for a `parseKeywordShortfall` call in `classify.ts` — there is none, and adding one would parse what that module just serialised. What Tasks 4 and 6 must respect is that CI runs classify's harvest and hands it to build via `--harvest-from`, so a message added to build's own search path alone is a message the daily run never prints.
...
**Every row is a POINT-IN-TIME reading, and the live source is the build log.** The total moves ~3.5 names an hour, so a dated figure here is stale within a day by construction and chasing it just re-creates the drift this table already had once. Every harvest re-measures it: `classify:` prints the enumerated/required pair and both shortfall terms on each run. Read that before sizing anything; the table is for orientation. `PARTITION_KEYWORDS`' comment owns the reading series and its dates, per `CLAUDE.md`; no reading is copied here, because that is how this table drifted once already. **Re-measure with one `size=1` search rather than trusting any figure below**, and do so before sizing anything if more than a week has passed.
...
**Read that 60.5% correctly, and read what it does NOT mean.** Publisher cells recover *less* than the refinement list **on `keywords:deepseek-harness`**, and this plan does not claim otherwise about that keyword. The second keyword is a different measurement and inverts the asymmetry — see "The second keyword is why this plan has a date" below; every unqualified statement in this section is about the first keyword. The 95 names are a **subset** of the 156 the refinement cells already reach, so landing this axis adds **zero** to `recovered` in the steady state — `recovered` counts names in the union past the window, and a cell re-supplying a name the union already holds does not move it. Two measurements taken 2026-09-08 say how small the steady-state gap is:

- Of the 5,250 names inside the window, exactly **one** carries `deepseek-harness` and none of the 26 refinement keywords (`dsh-deckseek`) — and that one is inside the window, so it is covered anyway. The refinement list tag-covers 5,249 of 5,250. Twenty-SIX, not 27: `PARTITION_KEYWORDS` has 27 entries but one of them IS `deepseek-harness`, which every result carries by definition and which `partitionKeyword` self-skips. Counting it makes the measurement tautological — `PARTITION_KEYWORDS`' comment records a first pass that did exactly that and reported a meaningless 500 of 500 covered.
- **Do not combine that figure with the 2.8% uncovered at ranks 5,000–5,250** in `PARTITION_KEYWORDS`' comment. The two samples were taken against different refinement lists (the 2.8% predates `deepwatch`) over different populations — the bottom 250 versus the whole window — and neither has been re-measured against the other's. The design doc's drift arithmetic multiplies the 2.8%; this bullet does not.
...
Neither result argues against this plan; against THIS keyword its value was never the steady state. They do fix what "working" looks like, which is why the verification section below does not ask for `recovered` to rise on either keyword — it asks for a name only a publisher cell supplied. Their value is the failure mode refinements cannot reach: the `sayedev` event was 20 packages released together, **six** of them past the window with no shared refinement and 14 already visible (6 + 14 = 20; the residual read 7 that day because the total also climbed mid-run) — so `maintainer:sayedev` was derivable from what the harvest had already read and its cell recovers all 20. A refinement list reaches that family only if a human adds the tag after the build has gone red.

### The second keyword is why this plan has a date
...
**Do not multiply that 4.4% by the tail to get a date.** That is the drift model `PARTITION_KEYWORDS`' comment and the design doc both refute, and it over-predicted fivefold where it could be checked (`deepseek-harness`'s tail grew 151 → 183 with the residual pinned at 1; 2.8% × 183 predicts 5). The step reading is the operative one and it is harsher: the 81 sit under 38 owners with `huanlin` holding 21 — twice `MAX_UNREACHABLE_RESIDUAL` — so one family landing past the window together reddens the build that day, the `sayedev` shape again. The rate bounds how fast uncovered names enter the tail, which is what makes the unit **days**: as an ordering estimate, the first red build (the throw is `>`, so eleven names) needs a ~250-name tail, about four days past the crossing at 60/day, and two to seven on the sample's Wilson interval (2.5%–7.7% at 11 of 250; the Wald interval, 1.9%–6.9%, under-covers at that count). And note which guard fires: at 4.4% refinement recovery is still 95.6%, above `MIN_UNREACHABLE_RECOVERY`, so the absolute cap is what breaks. **A refinement cannot close it** — those 81 packages carry `dsh-plugin` and mostly nothing else, so there is no second tag to intersect on.

This axis can, and it reaches **all 81**: every one carries at least one maintainer username inside Task 1's grammar, so a cell can be built for each. That is reachability and not a coverage win — the 81 are defined as the names no refinement reaches, so "81 of 81 by refinement" is the definition restated, and the first bullet above records the same trap costing a first pass a meaningless "500 of 500". Over the shared population, this keyword's 3,973 names, the two axes are 97.96% against 100%. A neutral set — one defined by neither axis, as npm's `from` cap defines the 157 — is what the 60.5%/99.4% pair had and what `dsh-plugin` will not have until it crosses. What the result does establish is the asymmetry: refinements have no move against these 81, and this axis has one. Concentration describes the uncovered **set** — **38** distinct maintainers, the seven largest cells reaching 51 (63%), a minimum cover 37 cells, `huanlin` alone owning 21 with `keywords:dsh-plugin maintainer:huanlin` probing to 26. **Those 37 are not this plan's cost.** They are oracle-selected, and Task 5 has no oracle: its loop probes every vocabulary entry (`for (const maintainer of publishers)`), so the price is the vocabulary and the paging half is unbounded — see Task 5's Step 3 warning, which sizes it at roughly tripling the npm half of a run. Size the axis from there, never from 37. So for the second keyword this plan is not insurance against an occasional burst: it is the only one of the two axes with a move against a residue arriving on a schedule. **Being the only one with a move is not the same as being sufficient**, and the limitation stated in the Architecture paragraph is what bites: a cell exists only for a publisher already SEEN. Every one of these 81 is in-window today, which is precisely why all 38 owners are seen — the reachability result rests on the condition that ends at the crossing. The one measurement over a real past-window tail is still 60.5% (first keyword), and a family landing entirely past the window is invisible to `maintainer:` exactly as it is to `keywords:a,b`: both are queries, and the `from` cap is what neither escapes. Size this plan as the best available query-side answer, not as coverage.

**Read the username off `maintainers[].username`, which is what Task 2 does.** `publisher.username` is the last publishing identity and can be a CI bot's display name — `GitHub Actions` for 12 of these 81 — which is not an npm account and which `maintainer:GitHub Actions` answers 0 for. A first pass at this measurement read that field and reported 31 owners with a 70% top-seven; both were artefacts of the wrong field, and the correction strengthens the case rather than weakening it.

Two consequences for the tasks below. Task 5 gates publisher cells on a keyword being past the window, which is correct and means `dsh-plugin` buys nothing until it crosses — the vocabulary should nonetheless be **seeded from it now**, which Task 4 already does by reporting publishers from every search the harvest reads. And do **not** turn the crossing into an acceptance test on `recovered`. After `dsh-plugin` crosses, `recovered` rises whether or not this plan landed: the window sweep plus the 22 non-empty refinement cells supply the tail on their own. Accepting on it would repeat verbatim the mistake the verification section refuses ("An earlier draft of this section did, and it is the wrong test"), relabelled onto the second keyword. The test is the same on both keywords and it is about provenance, not magnitude: a name in the union that **only** a publisher cell supplied. What the second keyword changes is how often that test has something to find — from a family event to a standing condition — not what the test is.
...
- Produces:
  ...
  Amended in review. This task shipped `seen: Iterable<string>` with no
  write-side validation and no count bound, and all three were closed on the
  branch before merge: a bare `string` IS an `Iterable<string>`, so
  `mergePublishers(state, 'sayedev')` type-checked and merged six
  single-character publishers into a vocabulary documented as never removing;
  `parsePublisherState` threw on a name the write path would happily commit, so
  one unfiltered producer could write a file every subsequent build refused to
  read; and `MAINTAINER_MAX_LENGTH` was the per-name half of a two-bound
  convention every other npm-sourced list in this repo satisfies. Task 6's
  wiring is unaffected — it passes an array — and `parsePublisherState` now
  collapses a duplicate rather than round-tripping one, which the interface's
  own "sorted, unique" already claimed.
 ...
 * 2026-09-08 and each costs one `size=1` request — 3,390 once `dsh-plugin` is
 * seeded as "The second keyword" above requires, leaving 610 under this
 * ceiling rather than 959. Sized above today's vocabulary so nothing is
 * skipped yet, and bounded so a year of accumulation
...
**Do NOT accept on `recovered` having risen — on either keyword.** An earlier draft of this section did, and it is the wrong test: `recovered` is `max(0, enumerated - SEARCH_WINDOW)`, so a publisher cell moves it only by contributing a name the union does not already hold — i.e. only by finding the residual itself. Measured 2026-09-08, the residual is 1, a 115-cell publisher probe found none of it, and it may not be a package at all (npm's `total` counting one document search will never serve is not excluded). On `keywords:deepseek-harness` a correct implementation of this plan will leave `recovered` unchanged on an ordinary day. On `keywords:dsh-plugin` after it crosses, `recovered` rises with or without this plan — the window sweep and the 22 non-empty refinement cells supply that tail — so the signal is the same on both keywords and it is provenance, not magnitude: a name in the union that only a publisher cell supplied. What the plan buys on the first keyword is the day that is not ordinary; what it buys on the second is a standing condition instead of an event. Both are covered by the same two deliverables:
