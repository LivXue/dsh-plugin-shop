...
**Goal:** Build the dsh-plugin-store catalog pipeline — harvest dsh plugins from npm by keyword, gate and tier them, and emit deterministic static JSON plus a committed integrity snapshot.
...
**Spec:** `docs/design/2026-08-18-dsh-plugin-store-design.md`
...
```json
{
  "name": "dsh-plugin-store-registry",
  "private": true,
  "type": "module",
  ...
  build:
    ...
    steps:
      ...
      - name: Commit the snapshot
        ...
        run: |
          git config user.name 'dsh-plugin-store bot'
