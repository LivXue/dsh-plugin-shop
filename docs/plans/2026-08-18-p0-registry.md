...
The impure shell. Everything that touches the network or the filesystem lives here and nowhere else.
...
Network and filesystem access live in these two modules and nowhere else, so
every policy decision stays in a pure function a fixture can drive."
```
