...
**Files:**
- Modify: `.github/workflows/daily.yml:56-70` (the classifier commit step), `:124-144` (the snapshot commit step)
...
3. *The shallow clone was an unverified premise, and the test written to re-check it does not.* `.github/workflows/daily.yml:48` is `actions/checkout@v4` with no `fetch-depth`, i.e. **depth 1**, and `git rebase origin/main` in a shallow repository is not obviously sound. It was simulated before this amendment: `git clone --depth 1`, a second clone advances `origin/main`, then the function body verbatim — `git fetch origin main` deepens far enough that the graft *is* the merge base, the rebase succeeds, and the push lands on attempt 1. **`fetch-depth: 0` is therefore not required.** This mattered enough to check first: a rebase that could never succeed would turn an intermittently-landing push into one that never lands and annotates `::error::` every morning — a fix strictly worse than the bug.
