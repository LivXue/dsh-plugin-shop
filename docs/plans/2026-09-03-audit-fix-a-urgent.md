...
**Scope note, AMENDED 2026-09-03 after Task 2's review.** This task now DOES drop the backup argument from `build.ts:60`'s `searchByKeywords(fetch, undefined, npmToken, npmBackupRegistry)` call — one argument, in a file this task already edits. Measured: `registry.npmmirror.com/-/v1/search` answers `{"objects":[],"total":0}` for both harvest keywords while answering `total=10000` for `text=react`, so it does not implement the `keywords:` qualifier. The original scope note (below) argued the deferral was safe because Task 1's coverage check makes a mid-pagination source switch loud. That argument holds for a MIXED index, but not here: a numeric `total: 0` means `searchTotal` does not throw, the keyword reports unpartitioned, the empty first page breaks the loop, and the coverage floor becomes `min(0, 0) = 0`, which passes — a zero-name harvest with a green build. Only a hand-run `pnpm build:catalog` without `--harvest-from` reaches it (the daily workflow always passes that flag), but that is precisely the command a maintainer runs to see the fetching layer work end to end. Also update `classify.ts`'s copy of the empty-string claim if Task 2 has not already. The rest of the original note stands:

**Original scope note.** D-6's third sub-fix — "never fail over a paged search" — is deliberately **not** in this task. It contradicts the passing test at `npm-client.test.ts:600` ("searches through the failover too"), and Task 1's coverage check now makes a mid-pagination source switch loud rather than silent (a mixed index enumerates fewer distinct names than the keyword's total and throws). Deferred to WP3 with that reason.
...
**Files:**
- Modify: `.github/workflows/daily.yml:56-70` (the classifier commit step), `:141-170` (the snapshot commit step). Line numbers are as of `e8b40f5`'s parent; this task's own commits shift everything below the classifier step.
- Test: `registry/scripts/tests/workflow.test.ts` (created in Task 5, restructured in its fix round `3e1d877`; one `describe` block, 10 tests)

**Interfaces:**
- Consumes: `workflow.test.ts`'s module-scope `buildSteps` (a `WorkflowStep[]` of `{ name, run }`) from Task 5's fix round. The `stagedPaths` helper this task's first draft named **never shipped** — the fix round replaced the flat line scan with a structural YAML parse, and `gitAddPaths(run, label)` is its nearest equivalent. Do **not** reintroduce a `workflow.split(/\n      - /)` scan: that is the exact indentation-fragile, step-unscoped idiom the fix round removed, and it is why moving a `git add` into the wrong step stayed green.
- Produces: no new exports; the workflow's two push steps gain a `push_with_rebase` shell function

**Amended 2026-09-03, before dispatch — two defects in this task's own first draft:**

1. *It asserted strings, not behaviour.* `expect(step).toContain('git fetch origin main')` passes if those words sit in a comment, in a dead branch, or in a function nothing calls. Task 5's review spent an entire fix round removing that class of assertion from this very file; reintroducing it one task later would be indefensible. The failure E-2 describes is behavioural — a push is rejected and nothing retries — so the test runs **the shipped shell, extracted from `daily.yml` itself**, against real local repositories. A transcribed copy of the function in the test would agree with whatever wrong assumption produced it, which is the 0.5.0–0.5.2 failure mode CLAUDE.md records.
2. *Its own step-finder could not survive its own fix.* `pushSteps` matched `git push` only at the start of a line — written against today's bare `git push`, never re-evaluated against the `if git push origin HEAD:main; then return 0; fi` that Step 3 introduces two pages later. The two are mutually exclusive by construction, so after Step 3 `pushSteps` was `[]`, all eight behavioural cases (they live in `for (const step of pushSteps)`) were never generated, and `keeps the two copies byte-identical` passed *vacuously* — `[].map(…)` destructures to `[undefined, undefined]` and `expect(undefined).toBe(undefined)` holds. Exactly one assertion out of ten would have noticed. Found by the implementer, who stopped rather than widening the regex and carrying on; verified here against the parsed YAML before amending. The predicate is now unanchored, and a module-scope `throw` makes an empty `pushSteps` fail the file instead of quietly generating no tests — the same reflex as `findStep` throwing on 0 or >1 rather than picking the first.

3. *The shallow clone was an unverified premise, and the test written to re-check it does not.* `.github/workflows/daily.yml:48` is `actions/checkout@v4` with no `fetch-depth`, i.e. **depth 1**, and `git rebase origin/main` in a shallow repository is not obviously sound. It was simulated before this amendment: `git clone --depth 1`, a second clone advances `origin/main`, then the function body verbatim — the rebase succeeds and the push lands on attempt 1.

*The mechanism stated here was wrong, corrected after review.* This paragraph said the fetch "deepens far enough that the graft is the merge base". It does not deepen at all. Measured with origin genuinely advanced 5 → 6 commits: the graft boundary is byte-identical before and after `git fetch origin main`, and `merge-base(HEAD, origin/main)` **is** the graft. The real reason is stronger than the one claimed — `origin/main` only ever moves *forward* from the tip the runner checked out, so the merge base is the graft **by construction**, and the shallow clone is safe structurally rather than by the fetch's generosity. The one case that defeats it is a history rewrite on `main` that orphans the graft, where no merge base exists and the rebase fails loudly through `::error::`. The conclusion is unchanged; the reasoning a future reader would apply is not. **`fetch-depth: 0` is therefore not required.** This mattered enough to check first: a rebase that could never succeed would turn an intermittently-landing push into one that never lands and annotates `::error::` every morning — a fix strictly worse than the bug.

*Corrected after review.* This paragraph then claimed Step 1's "pushes cleanly when origin moved under it" case **is** that reproduction, "so the premise is re-checked on every run instead of being trusted from this paragraph". **That was false.** `sandbox()` pushed exactly one commit before `git clone --depth 1`, so the shallow boundary *was* the root commit, the clone held the entire history, and the case passed identically against a non-shallow clone — it could not discriminate the hazard it was named for. A test that passes for the wrong reason, in the same class as the audit's own H-1 to H-3. The premise itself survives: the reviewer verified it independently against a 40-commit origin with a real `actions/checkout@v4`-shaped fetch (`git init` + `remote add` + `fetch --depth=1`), where the boundary is a genuine graft. `sandbox()` must seed several commits before the shallow clone so the reproduction is real.

4. *`git push origin HEAD:main` made a previously-harmless case dangerous.* Both commit steps run on `workflow_dispatch`, which fires on **any** ref, and neither carries a `github.ref` guard. The old bare `git push` went to the *dispatched* branch's upstream, so a dispatch from a feature branch was harmless. Retargeting the rebase and the push to `main` means a dispatch from any additive branch replays that branch onto `main` and pushes it — unreviewed content on `main`, step exit 0, no annotation — and the depth-1 checkout makes it worse, since the grafted tip means the replayed diff is the whole worktree. On a branch that conflicts with `main` (the common case) the other outcome is two `::warning::` and one `::error::` on **every** dispatch, which corrodes the exact annotation this task relies on for visibility. Reproduced in review. `HEAD:main` buys nothing on `main`, where a bare `git push` after the rebase behaves identically. The fix is the guard every other side-effecting step and job in this file already carries — **four** of them, at `:134` and `:154` (steps) and `:210` and `:253` (the `publish` and `deploy` jobs), post-`b6a64de` line numbers. Only `Publish to Pages` carries the comment "`workflow_dispatch` fires on ANY ref"; `:134`'s comment is about `dist/` being git-ignored. So: `&& github.ref == 'refs/heads/main'` on both `if:` conditions, plus a test pinning it so it cannot be dropped silently.

- [ ] **Step 1: Write the failing test**

> **Superseded by what shipped — read this before copying the block below.** This is the state Task 6 was *dispatched* with. Review found four defects in it, fixed in `b6a64de`, and the block is deliberately **not** re-transcribed, because a second copy is what drifts. Take the shipped file as the authority (`e8b40f5`, then `b6a64de`). Relative to it, the block below lacks: the ref-guard test and the `if` field on `WorkflowStep`/`parseBuildSteps` (amendment 4); a `sandbox()` seeded with `ORIGIN_COMMITS = 40` plus the shallowness assertion that makes the graft real (amendment 3) — it still shows one `base` commit, which cannot tell a shallow clone from a full one; `GIT_CONFIG_GLOBAL`/`GIT_CONFIG_SYSTEM: '/dev/null'` in `IDENT`, without which three ordinary developer settings fail seven cases; `stdio: 'pipe'` on the `git` helper; and a `try`/`catch` around `sandbox()`'s body, whose absence leaked 24 directories observed in `/tmp`. It still carries the `core.hooksPath` line that `GIT_CONFIG_GLOBAL` made redundant.

Extend the import line at `registry/scripts/tests/workflow.test.ts:28` and add three more:
...
Run: `npx vitest run registry/scripts/tests/workflow.test.ts -t "pushes safely"`
Expected: `Tests  9 failed | 1 passed | 8 skipped (18)`. The nine error with `daily.yml: the "Commit the classifier's output" step defines no push_with_rebase()`, thrown by `pushFunction`, because the step still ends in a bare `git push`. The eight skips are Task 5's tests, filtered out by `-t`.

The one that **passes** at RED is `has exactly the two committing steps that push, and no third` — today's two bare pushes are exactly the two expected names. It is the only case here that must go on passing at GREEN rather than flipping, so if it ever fails at GREEN while the others go green, the predicate has stopped seeing the pushes it is supposed to find. That inversion is precisely how this task's first draft was caught.

**This RED is weaker than it looks and must be strengthened.** All ten cases failing on one missing definition proves the extraction runs, not that the behavioural cases discriminate. So before Step 3, run each behavioural case against a *deliberately wrong* function and watch it fail — the mutation check CLAUDE.md requires. Temporarily replace `pushFunction`'s body with `return 'push_with_rebase() {\n  git push origin HEAD:main\n}'` (today's behaviour, no fetch, no rebase, no retry) and confirm:

- "pushes cleanly when origin moved under it" FAILS (status 1, the push is rejected)
- "retries once when the push is rejected" FAILS (status 1, zero `::warning::`)
- "fails loudly when the rebase cannot be completed" FAILS (no `::error::`)

Then restore `pushFunction`. A behavioural test that passes against the code it is supposed to condemn is not a test.
...
- Modify: `registry/scripts/src/gate.ts:13-21` (constants), `:82-88` (license and repository checks)
- Modify: `registry/scripts/src/npm-client.ts:364` (`PEERS_MAX_COUNT`), `:440-441` (the peers projection)
- Modify: `registry/scripts/src/github-client.ts:596-606` (`fetchRepoCandidate`'s manifest fetch and parse; the function opens at `:584`)

> **Anchors re-measured 2026-09-03, after Tasks 1-4.** The two client files were rewritten by this
> plan's own earlier tasks, so the ranges written when it was drafted are stale by hundreds of lines:
> `PEERS_MAX_COUNT` was cited at `:173-183` and is at `:364`; the peers projection was cited at
> `:259-261` and is at `:440-441`; `github-client.ts:511-534` now holds the subpackage probe, not the
> manifest read. `schema.ts:11` and `gate.ts:82-88` are unchanged and still correct. Verify any
> anchor before trusting it — this plan has already shipped one task against a wrong line range.
...
> *dispatched* with; review found blocking defects in it across several fix rounds. The
> blocks below are deliberately **not** re-transcribed — a second copy is what drifts, and it already
> drifted once in Task 6. Take the shipped files as the authority — `b9a48c6` and every `fix(registry):` commit after it — **read `git log --oneline b9a48c6..HEAD -- registry/` rather than any list written here**. An enumerated list has now gone stale three times, twice in the very commit that was fixing it, so this block no longer keeps one.
