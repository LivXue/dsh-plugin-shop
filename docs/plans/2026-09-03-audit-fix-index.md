...
| [C — network, artifacts, CI](2026-09-03-audit-fix-c-network-artifacts-ci.md) | Weather in the GitHub client, what Pages serves, the workflow's permissions and pins | 17 | 19 | A daily build + workflow changes |
...
| [E — close the test gaps](2026-09-03-audit-fix-e-test-gaps.md) | Tests that pass for the wrong reason, proven by injecting the break | 11 | 13 | Rides along with A–D |
...
**The four ids added on 2026-09-04 all have tasks now (2026-09-04).** D-10 and E-14 belong to plan C, H-10 and H-11 to plan E. E-14 was closed on the plan A branch, out of plan order, because it is the one of the four that can corrupt a registry file on main. The other three were written as C Task 19, E Task 12 and E Task 13, and the task counts in the table moved with them (C 18 → 19, E 11 → 13).

Two of the three are notes rather than fixes, and each records something that would otherwise be re-derived: **C Task 19 (D-10)** says a build stopped by the narrowed `no-manifest` must be answered by lowering `REPO_BACKFILL_BUDGET`, never by widening the status back, because widening restores the false-verdict bug in full. **E Task 12 (H-10) is CLOSED** — the finding's unconfirmed suspect was right: `@deepseek-ai/dsh-client-ui-settings-plugin-inventory` split 插件列表 in two between `0.1.1-rc.2` and `0.1.2-rc.1` and collapsed the Loader plane, so the hot mount always succeeded and the selector was looking inside a closed disclosure. **E Task 13 (H-11) corrects its own finding's premise**: the leak is not in `web-full-flow.e2e.ts`, which is balanced at 2 `mkdtempSync` / 2 `rmSync`, but in the host tests, where `index.test.ts` alone has 41 creations against 2 removals. Two of them are notes rather than bugs to fix: D-10 records why a build that stops must not be "fixed" by reverting the narrowing that made it stop, and H-10 records two hypotheses that were tested and disproved, so the next person does not retry them. **E-14 is the exception: it is closed on this branch**, out of plan order because it is the one of the four that can corrupt a registry file on main, and closing it needed no plan — `push_with_rebase` now refuses to push when its autostash did not reapply, with the case driven against real repositories in `workflow.test.ts`. Its row in the table above is unchanged — the count is of ids assigned to a plan, not of open ones, and re-deriving it per fix is how the coverage claim above stops being checkable. The rest of plan C is untouched.
