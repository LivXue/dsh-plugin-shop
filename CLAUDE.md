...
- **A search measures what it enumerated against the total it was promised, and says what is missing.** One npm query reaches 5,250 names (`from` is capped at 5,000, and a larger one silently returns page 0 — re-verified live 2026-09-07: `from=5000` serves a distinct tail, `from=5100` and above come back as page 0), so an over-window keyword is partitioned on `keywords:a,b` intersections — the only qualifier that splits by SUBJECT, and it has no negation, so a partition is never covering by construction. (`maintainer:` IS also a filter and composes with `keywords:`; it was measured as inert and that measurement was wrong. The publisher axis it opens is planned, not built — design doc, 2026-09-08 follow-up.) The harvest stops paging on the answered `total`, never on a short page, and a response carrying no `total` throws too. The GitHub half splits each topic into stars/date/size windows under the 1,000-result search cap and throws when one still exceeds it after every split.
  ...
  **A shortfall is not one quantity, and this is the one place a bound must not be shared.** It decomposes exactly into names missing from INSIDE the addressable window — where every rank is reachable in one query, so a miss is the registry serving short — and names missing from the tail beyond it, which no re-reading can close. The first is bounded at `MAX_SEARCH_SHORTFALL` (3) and throws. The second is bounded by a recovery rate and `MAX_UNREACHABLE_RESIDUAL` (25): **within it the catalog is knowingly published a few names short**, reported in the build report and the CI log, not thrown. That is a deliberate change from "throws rather than truncating", made because the API's own ceiling is a shortfall no list of refinements can close, and a red build every day is not a safer catalog. Sizing one constant for both is what made the daily build red on every tree; re-merging them is the regression to watch for.
