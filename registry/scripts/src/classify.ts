...
// Exact: `endsWith('classify.ts')` also admits `preclassify.ts` and
// `reclassify.ts`. `node -e` leaves process.argv[1] undefined, so basename('')
// matches no name and a bare import runs nothing.
