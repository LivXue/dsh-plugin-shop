 ...
 *   gate the same way (finding #5 of the PR #58 review — the caller used to
 *   pass repo-state.json's own record, peers still attached, and a
 *   peers-heavy repository crossed the payload budget here while it fit in
 *   the build). The caller reads them from committed `repo-state.json`
 *   rather than re-harvesting, which costs no GitHub call and cannot advance
 *   the harvest state. A repository discovered by today's build is therefore
 *   classified by tomorrow's run — the same "unclassified, retried next
 *   build" state D4 already defines.
