...
| | |
|---|---|
| **浏览与搜索** | 按 `dsh-plugin` 与 `deepseek-harness` 关键字，从整个公共 npm registry 采集的 **9,377 个插件**（外加把这些关键字用作 topic 的 GitHub 仓库）；作者若自己声明了简介就用作者原话，并归入七个分类 |
