...
dsh plugin --profile web add dsh-plugin-shop@0.8.1
# 或全程走 npx，什么都不装：
npx -y @deepseek-ai/dsh plugin --profile web add dsh-plugin-shop@0.8.1
```
...
```sh
dsh plugin --profile <profile> add dsh-plugin-shop@0.8.1
```
...
| | |
|---|---|
| **浏览与搜索** | 数千个插件，按 `dsh-plugin` 与 `deepseek-harness` 关键字从整个公共 npm registry 采集（外加把这些关键字用作 topic 的 GitHub 仓库）；作者若自己声明了简介就用作者原话，并归入七个分类 |
...
| **体积与作者** | 每张卡片显示装上它会占多少磁盘，紧挨着发布账号——用的是 npm 自己的解包体积，所以和该包的 npm 页面对得上 |
| **滤掉跑不起来的** | 声明的组件你这套安装没有提供的插件会被标为不兼容，类别栏末尾的筛选可以把它们从货架上拿掉。它不会隐藏名字已被别的安装占用的插件，也不会从「已安装」视图里减东西——那些卡片是唯一能说明问题、也是唯一能撤销它的地方 |
| **刷新** | 当卸载或启停已经在服务端生效、只是当前页面还停在旧状态时，商店提议刷新这个页面——不会自动刷新，因为刷新会丢掉进行中的工作 |
| **重启** | 插件的宿主半边没能热挂载时；以及它挂载成功、但插件带浏览器半边时——会话中途装上的插件，它的界面要重启 dsh 才进得了页面，所以商店直说，而不是给一个按了也不会变的刷新。提议前先讲清代价：页面会断开，进行中的工作会中断 |
| **自更新** | 搜索框旁显示商店自己的版本号，自动查 npm 有没有新版本，有则一键用钉死版本更新，随后照常重启 |

## 🔎 货架是怎么来的

目录不是谁手工维护的名单。每天跑六步，其中两步是这个货架值得一读的原因：

1. **全网爬取。** npm 上带 `dsh-plugin` 或 `deepseek-harness` 的包全部过一遍，外加把这些关键字
   用作 topic 的 GitHub 仓库。不需要提交，也没有排队。
2. **逐个筛查。** 采到的东西大多进不了货架；
   [仓库 README](https://github.com/LivXue/dsh-plugin-shop/blob/main/README.zh.md)
   顶部的实时徽章同时统计两边的数量。没有 `dsh.bundle`
   的是库，不是插件；没有许可证或仓库的无从审查；npm 上已废弃的直接出局；仓库来源还额外要求无构建
   脚本、无 `workspace:` 依赖——这两样任一都会让安装在你机器上失败；名字跟热门包只差一两个字符
   的会被扣住，等人核过才放。十七种记录在案的拒收理由，全是
   机械判定，每条拒收都附一句作者能读懂的话。
3. **分类与记录。** 七个分类，以及每个插件声明的 peer 模块——只记名字，从不记版本范围。
4. **发布。** 内容寻址的 JSON，同时发到 npm 与 GitHub Pages。
5. **在你这边拉取。** 本包竞速多个来源，并在信任任何字节之前校验 sha256。
6. **依赖检测也在你这边。** 你的安装拿那些 peer 名字去解析**你自己的** profile——跟 dsh 的
   loader 挂载时问的是同一个问题。解析不到的卡片显示**不兼容**并列出缺了什么。这个结论必须在你
   机器上算，因为答案取决于你实际跑的是哪个 harness，而不是构建当时跑的那个。

完整流程图在
[仓库 README](https://github.com/LivXue/dsh-plugin-shop/blob/main/README.zh.md#-整体是怎么串起来的)。

有一件事它**没有**做：没有任何一条收录被人读过。今天 `verified.yml` 是空的，全部条目都是社区
层，每次安装都会要求你确认。机械筛查不等于评审。
