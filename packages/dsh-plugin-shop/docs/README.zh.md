...
| **滤掉跑不起来的** | 必需的组件你这套安装没有提供，或者作者声明支持的 dsh 版本、profile 跟你正在用的不符，这样的插件会被标为不兼容，类别栏末尾的筛选可以把它们从货架上拿掉。它不会隐藏名字已被别的安装占用的插件，也不会从「已安装」视图里减东西——那些卡片是唯一能说明问题、也是唯一能撤销它的地方 |
...
3. **分类与记录。** 七个分类，每个插件必需的 peer 模块——只记名字，从不记版本范围——以及作者
   自己声明的 `dsh.compatibility`（如果有）。
4. **发布。** 内容寻址的 JSON，同时发到 npm 与 GitHub Pages。
5. **在你这边拉取。** 本包竞速多个来源，并在信任任何字节之前校验 sha256。
6. **依赖检测也在你这边。** 你的安装拿那些 peer 名字去查**你自己的** profile，再剔除 dsh 网页端
   自己就提供的模块。两边都没有的，卡片显示**不兼容**并列出缺了什么；作者声明支持的 dsh 版本或
   profile 跟你正在用的不符，卡片同样显示**不兼容**，并写明声明的是什么、你实际运行的是什么。
   这个结论必须在你机器上算，因为答案取决于你实际跑的是哪个 harness，而不是构建当时跑的那个。
